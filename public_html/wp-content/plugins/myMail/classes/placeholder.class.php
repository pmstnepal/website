		...
		if($count = preg_match_all('#\{(share:(twitter|facebook|google|linkedin) ?([^}]+)?)\}#', $this->content, $hits)){
			...
			for($i = 0; $i < $count; $i++){
				...
				if(isset($mymail_social_services[$service]))
					...
					$content = apply_filters('mymail_share_button_'.$service, '<img src="'.MYMAIL_URI.'/assets/img/share/share_'.$service.'.png" />');
					$placeholders[$hits[1][$i]] = '<a href="'.$_url.'" class="social">'.$content.'</a>'."\n";
