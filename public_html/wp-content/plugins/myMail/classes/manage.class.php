...
';
		$html .= '<p>'.__('Import as', 'mymail').':<br><label><input type="radio" name="status" value="subscribed"> '.__('Subscribed', 'mymail').'</label> <label><input type="radio" name="status" value="unsubscribed"> '.__('Unsubscribed', 'mymail').'</label><br>&nbsp;&nbsp;<label><input type="checkbox" id="keepstatus" name="keepstatus" value="1"> '.__('don\'t overwrite status if user exists', 'mymail').'</label></p>';
		$html .= '<p>'.__('Existing subscribers', 'mymail').':<br><label> <input type="radio" name="existing" value="skip" checked> '.__('skip', 'mymail').'</label> <label><input type="radio" name="existing" value="overwrite"> '.__('overwrite', 'mymail').'</label><input type="radio" name="existing" value="merge"> '.__('merge', 'mymail').'</label></p>';
	...
	public function ajax_do_import() {
		...
		$allmails = $wpdb->get_results("SELECT post_title, ID, post_status FROM {$wpdb->posts} WHERE {$wpdb->posts}.post_type = 'subscriber' ", OBJECT_K);
		...
		if($list){
			foreach($list as $line){
				...
				if($bulkdata['keepstatus']){
					$status = (isset($allmails[$email])) ? $allmails[$email]->post_status : $bulkdata['status'];
				}
				
				$result = $mymail_subscriber->rawinsert($ID, $email, $status, $userdata, $userlists, $bulkdata['autoresponder']);
