...
////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////Extended by SL
/////////////////////////Added Feature: Display Time in Nepali Font + human readable
////////////////////////////////////////////////////////////////////////////////////////
     ...
     * @return string
     */
    public function get_day_of_npweek($day)
    {
        switch ($day) {
            case 0:
                $day = "आइतवार";
    ...
    public function eng_to_nep($yy, $mm, $dd)
    {
        ...
        if ($this->is_range_eng($yy, $mm, $dd) == false) {
            ...
            $this->nep_date["day"] = $this->get_day_of_npweek($day);
     ...
     * @return mixed
     */
    public static function convertTime($timestamp, $show_time = false, $human_readable = false)
    {
        $etn = new EngToNep();
        $current_time = time();
        /*if ($human_readable) {
            ...
            $time_diff_in_hour = $time_diff/60/60;
            $time_diff_in_day = $time_diff/60/60/24;
            if ($time_diff < 60) {
                $arr[] = floor($time_diff);
                $result = $etn->convert_to_nepali_font($arr);
                $nep_date_string = $result[0] . ' सेकण्ड अगाडि';
                return $nep_date_string;
            } elseif ($time_diff_in_minute < 60) {
                $arr[] = floor($time_diff_in_minute);
                $result = $etn->convert_to_nepali_font($arr);
                $nep_date_string = $result[0] . ' मिनट अगाडि';
                return $nep_date_string;
            } elseif ($time_diff_in_hour < 24) {
                $arr[] = floor($time_diff_in_hour);
                $result = $etn->convert_to_nepali_font($arr);
                $nep_date_string = $result[0] . ' घण्टा अगाडि';
                return $nep_date_string;
            } elseif ($time_diff_in_day < 3) {
                $arr[] = floor($time_diff_in_day);
                $result = $etn->convert_to_nepali_font($arr);
                $nep_date_string = $result[0] . ' दिन अगाडि';
                return  $nep_date_string;
            }
        }*/
        ...
        //convert to nepali time
        $date_arr = $etn->eng_to_nep($year, $month, $day);

        $date_arr['hour'] = $hour;
        $date_arr['minute'] = $min;
        $date_arr['sec'] = $sec;
        if ($am_pm == 'am') {
            $date_arr['am_pm'] = 'बिहान';
        } else {
            $date_arr['am_pm'] = 'बेलुकी';
        }

        $nep_date = $etn->convert_to_nepali_font($date_arr);

        $nep_date_string =   $nep_date['nmonth'] . ' ' . $nep_date['date'] . ' गते, ' . $nep_date['year'];

        if ($show_time) {
            $nep_date_string .= ' - ' . $nep_date['hour'] . 'ः' . $nep_date['minute'];
        }
	    ...
	    public static function convertYear($timestamp, $show_time = false, $human_readable = false)
    {
        $etn = new EngToNep();
        $current_time = time();
     ...
     * @return mixed
     */
    public function convert_to_nepali_font($arr)
    {
        if (!empty($arr)) {
            foreach ($arr as $key=>$item) {
                if ( is_numeric($item)) {
                    $new_char = '';
                    $ch_arr = str_split($item);
                    foreach ($ch_arr as $i) {
                        switch ($i) {
                            case 0:
                                $new_char .= '०';
