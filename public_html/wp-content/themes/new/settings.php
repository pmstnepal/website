			...
			<tr valign="top">
				<th>
					<label for="<?php echo $this->tag; ?>[<?php esc_attr_e( $key ); ?>]">
						<?php _e( is_array( $detail ) ? $detail['label'] : $detail, 'content' ); ?>
					</label>
				</th>
				<td>
					<?php if ( isset( $detail['input'] ) && $detail['input'] == 'textarea' ) : ?>
					<textarea class="regular-text" cols="100" rows="15" id="<?php echo $this->tag; ?>[<?php esc_attr_e( $key ); ?>]" name="<?php echo $this->tag; ?>[<?php esc_attr_e( $key ); ?>]"><?php if ( array_key_exists( $key, $this->options ) ) { esc_html_e( $this->options[$key] ); } ?></textarea>
					<?php else : ?>
